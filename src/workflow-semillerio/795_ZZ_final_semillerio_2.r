...
PARAM$semilla_primos  <- 600043  # se utiliza para generar el vector de  PARAM$semillerio  semillas
...
GenerarKaggle  <- function( tb_prediccion, modelo_rank, iteracion_bayesiana, sem )
{
  ...
  for( campo  in  c( "prob" ) )
  {
    ...
    for( corte in cortes )
    {
      ...
      if( sem > 1 )
      {
        nom_old  <- paste0( PARAM$experimento,
                            "_",
                            sprintf( "%02d", modelo_rank ),
                            "_",
                            sprintf( "%03d", iteracion_bayesiana ),
...
for( modelo_rank in  PARAM$modelos_rank )
{
  ...
  for( vsemilla  in ksemillas ) #recorro las semillas del Semillerio
  {
    ...
    if( ! future_con_clase )  GenerarKaggle( tb_prediccion, modelo_rank, iteracion_bayesiana, sem )
